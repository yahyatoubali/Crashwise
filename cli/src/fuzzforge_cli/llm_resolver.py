"""
Centralized LLM client resolver for FuzzForge CLI.
...
def _resolve_credentials(
    ...
    if not allowed:
        ...
        raise PolicyViolationError(
            ...
            f"Run 'ff oauth setup -p {provider}' to authenticate."
        )
    ...
    if not api_key:
        ...
        if _get_oauth_token(provider):
            raise LLMResolverError(
                ...
                f"Run 'ff oauth setup -p {provider}'"
            )
        raise LLMResolverError(
            f"No credentials found for provider '{provider}'. "
            f"Set environment variables or run 'ff oauth setup -p {provider}'"
        )
...
def check_provider_available(provider: str) -> Tuple[bool, Optional[str]]:
    ...
    # Not available
    return False, f"No credentials found. Run 'ff oauth setup -p {provider}'"
