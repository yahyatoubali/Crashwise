...
def setup(
    ...
    Examples:
        cw oauth setup --list-providers              # Show available providers
        cw oauth setup -p openai_codex               # Setup OpenAI Codex
        cw oauth setup -p gemini_cli                 # Setup Gemini CLI
        cw oauth setup -p openai_codex --export-to-env  # Less secure
    """
    ...
    if list_providers:
        ...
        console.print(table)
        console.print("\n[dim]Usage: cw oauth setup -p <provider_id>[/dim]")
        raise typer.Exit(0)
...
def status(
    ...
    Examples:
        cw oauth status              # Human-readable table
        cw oauth status --json       # Machine-readable JSON
    """
    storage = get_storage()
    storage_info = storage.get_storage_info()
...
def remove(
    ...
    Examples:
        cw oauth remove -p openai_codex
        cw oauth remove -p openai_codex --force  # No confirmation
    """
    provider_id = provider.lower()
    if provider_id not in OAUTH_PROVIDERS:
...
def logout(
    ...
    Examples:
        cw oauth logout -p openai_codex
        cw oauth logout -p openai_codex --force  # No confirmation
    """
