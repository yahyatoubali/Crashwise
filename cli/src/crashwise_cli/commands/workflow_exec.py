...
from crashwise_sdk import CrashwiseClient, WorkflowSubmission
...
def get_client() -> CrashwiseClient:
    """Get configured Crashwise client with retry on network errors"""
...
def execute_workflow(
    ...
    try:
        with get_client() as client:
            ...
            console.print(f"\n💡 Monitor progress: [bold cyan]crashwise monitor live {response.run_id}[/bold cyan]")
            console.print(f"💡 Check status: [bold cyan]crashwise workflow status {response.run_id}[/bold cyan]")

            # Suggest --live for fuzzing workflows
            if not live and not wait and "fuzzing" in workflow.lower():
                console.print(f"💡 Next time try: [bold cyan]crashwise workflow run {workflow} {target_path} --live[/bold cyan] for real-time monitoring", style="dim")
            ...
            if live:
                ...
                try:
                    ...
                    console.print(f"💡 You can still monitor manually: [bold cyan]crashwise monitor live {response.run_id}[/bold cyan]")
                ...
                try:
                    ...
                    if final_status.is_completed:
                        ...
                        if fail_on:
                            try:
                                ...
                                if should_fail_build(findings.sarif, fail_on):
                                    ...
                                    console.print(f"💡 View details: [bold cyan]crashwise finding {response.run_id}[/bold cyan]")
                                    raise typer.Exit(1)
                                else:
                        ...
                        if not fail_on and not export_sarif:
                            console.print(f"💡 View findings: [bold cyan]crashwise findings {response.run_id}[/bold cyan]")
...
def workflow_status(
    ...
    try:
        ...
        if status.is_running:
            console.print(f"\n💡 Monitor live: [bold cyan]crashwise monitor live {execution_id}[/bold cyan]")
        elif status.is_completed:
            console.print(f"💡 View findings: [bold cyan]crashwise finding {execution_id}[/bold cyan]")
        elif status.is_failed:
            console.print(f"💡 Check logs: [bold cyan]crashwise workflow logs {execution_id}[/bold cyan]")
...
def workflow_history(
    ...
    try:
        ...
        console.print("\n💡 Use [bold cyan]crashwise workflow status <execution-id>[/bold cyan] for detailed status")
