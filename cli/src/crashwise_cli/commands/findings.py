...
def get_findings(
    ...
    try:
        ...
        if format == "json":
            ...
            console.print(
                f"\n💡 View full details of a finding: [bold cyan]cw finding show {run_id} --rule <rule-id>[/bold cyan]"
            )
            console.print(
                f"💡 Export these findings: [bold cyan]cw findings export {run_id} --format sarif[/bold cyan]"
            )
            console.print(
...
def show_finding(
    ...
    try:
        ...
        if not matching_finding:
            ...
            console.print(
                f"💡 Use [bold cyan]cw findings get {run_id}[/bold cyan] to see all findings",
                style="dim",
            )
            raise typer.Exit(1)
...
def display_finding_detail(finding: Dict[str, Any], tool: Dict[str, Any], run_id: str):
    ...
    console.print()
    console.print(
        f"💡 Export this run: [bold cyan]cw findings export {run_id} --format sarif[/bold cyan]"
    )
...
def findings_history(
    ...
    if not db:
        console.print(
            "❌ No Crashwise project found. Run 'cw init' first.", style="red"
        )
        raise typer.Exit(1)
    ...
    try:
        ...
        console.print(
            "\n💡 Use [bold cyan]crashwise finding <run-id>[/bold cyan] to view detailed findings"
        )
...
[bold]By Workflow:[/bold]"""
            ...
            console.print(
                Panel(
                    summary_text,
                    box=box.ROUNDED,
                    title="Crashwise Project Analysis",
                    border_style="cyan",
                )
            )
        ...
        console.print(
            "💡 View specific findings: [bold cyan]crashwise finding <run-id>[/bold cyan]"
        )
...
<html>
<head>
    <title>Crashwise Security Findings Report</title>
    <style>
...
<body>
    <h1>Crashwise Security Findings Report</h1>
    <div class="stats">
        <p><strong>Generated:</strong> {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
...
def findings_triage(
    ...
    Examples:
        cw findings triage abc123                    # Table output
        cw findings triage abc123 -f markdown        # Markdown report
        cw findings triage abc123 -p openai_codex    # Use specific provider
        cw findings triage abc123 --skip-llm         # Cluster only
    """
