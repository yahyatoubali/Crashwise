...
def init(
    ...
    """
    📁 Initialize a new Crashwise project
    """
...
def workflow_main():
    ...
    Examples:
        crashwise workflow run security_assessment ./target    # Execute workflow
        crashwise workflow status                          # Check latest status
        crashwise workflow history                         # Show execution history
    """
    pass
...
def finding_main(
    ...
    Examples:
        crashwise finding                            # Show latest finding
        crashwise finding <id>                       # Show specific finding
        crashwise finding show <run-id> --rule <id>  # Show specific finding detail
    """
    ...
    # Direct viewing: crashwise finding [id]
...
def version():
    ...
    console.print(f"Crashwise CLI v{__version__}")
    console.print("Short command: cw")
...
def main_callback(
    ...
    """
    🛡️ Crashwise CLI - Security testing workflow orchestration platform

    Quick start:
    • cw init                        - Initialize a new project
    • cw workflows                   - See available workflows
    • cw workflow <name> <target>    - Execute a workflow
    """
    if version:
        from . import __version__
        ...
        if len(args) >= 2 and args[0] == "finding":
            ...
            if not any(arg in ["--help", "-h", "--version", "-v"] for arg in args):
                if args[1] not in finding_subcommands:
                    # Direct finding display: cw finding <id>
                    from .commands.findings import get_findings
