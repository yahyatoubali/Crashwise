"""Secure cross-platform credential storage for Crashwise.

Supports:
- macOS: Keychain (Security framework)
- Linux: Secret Service API (libsecret) or file-based fallback
- Windows: Windows Credential Manager
- Fallback: ~/.config/crashwise/oauth with chmod 600
"""
...
class SecureStorage:
    ...
    def _get_fallback_path(self) -> Path:
        ...
        config_dir = Path.home() / ".config" / "crashwise"
    ...
    def _store_secret_service(self, account: str, token: str) -> None:
        ...
        try:
            ...
            collection.create_item(
                f"Crashwise: {account}", attributes, token, replace=True
            )
