"""
Enhanced exception handling for Crashwise CLI.

This module provides:
1. CLI-specific exceptions (ProjectNotFoundError, DatabaseError, etc.)
2. Re-exports from crashwise_sdk.exceptions for convenience
3. Backward compatibility aliases

Migration Path:
- New code should import from crashwise_sdk.exceptions directly
...
# =============================================================================
# These are the single source of truth from crashwise_sdk.exceptions
from crashwise_sdk.exceptions import (
    ErrorContext,
    CrashwiseError as _SDKCrashwiseError,
    CrashwiseHTTPError,
    DeploymentError,
...
# =============================================================================
# Backward Compatibility: CLI CrashwiseError
# =============================================================================
# Maintains compatibility with existing CLI code while delegating to SDK base


class CrashwiseError(_SDKCrashwiseError):
    """Base exception for Crashwise CLI errors.

    This class extends SDK CrashwiseError to maintain backward compatibility
    with existing CLI code that expects 'hint' and 'exit_code' attributes.

    New code should use crashwise_sdk.exceptions.CrashwiseError directly.
...
class ValidationError(_SDKValidationError):
    ...
    New code should use crashwise_sdk.exceptions.ValidationError.
    """
...
class ProjectNotFoundError(CrashwiseError):
    """Raised when no Crashwise project is found in current directory."""

    def __init__(self):
        super().__init__(
            message="No Crashwise project found in current directory",
            hint="Run 'cw init' to initialize a new project",
            exit_code=1,
        )


class APIConnectionError(CrashwiseError):
    """Raised when connection to Crashwise API fails."""
...
class DatabaseError(CrashwiseError):
    ...
    def __init__(self, operation: str, original_error: Exception):
        ...
        hint = "The database may be corrupted. Try 'cw init --force' to reset"
...
class FileOperationError(CrashwiseError):
...
def show_error(error: Exception, verbose: bool = False):
    ...
    """
    if isinstance(error, _SDKCrashwiseError):
    ...
    elif isinstance(error, CrashwiseError):
...
def handle_errors(func: Callable) -> Callable:
    ...
    Usage:
        @handle_errors
        def my_command():
            raise CrashwiseError("Something went wrong")
    """
    ...
    def wrapper(*args, **kwargs):
        ...
        except _SDKCrashwiseError as e:
            show_error(e)
            raise typer.Exit(1)
...
def require_project():
    """Ensure we're in a Crashwise project directory.
